...
def execute(filters=None):
    ...
    for emp in sorted(att_map):
        ...
        for day in range(filters["total_days_in_month"]):
            ...
            status_map = {"Present": "P", "Absent": "A", "Half Day": "HD", "On Leave": "L", "None": "",
                          "Holiday": "<b>H</b>"}
...
def get_attendance_list(conditions, filters):
    conditions2 = " and month(time) = %(month)s and year(time) = %(year)s"
    if filters.get("employee"): conditions2 += " and employee = %(employee)s"
    attendance_list = frappe.db.sql("""select employee, day(time) as day_of_month,
    concat(HOUR(min(TIME(time))),':',MINUTE(min(TIME(time))) ,'-',HOUR(max(TIME(time))),':', MINUTE(max(TIME(time))) ) as status
      from `tabEmployee Checkin` where 1 = 1 %s group by  employee ,day_of_month order by employee, time """ %
                                    conditions2, filters, as_dict=1)
    att_map = {}
...
def get_attendance_years():
    year_list = frappe.db.sql_list(
        """select distinct YEAR(time) from `tabEmployee Checkin` ORDER BY YEAR(time) DESC""")
    if not year_list:
