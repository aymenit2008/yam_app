...
def  get_rootwise_opening_balances(filters, report_type):
	...
	gle = frappe.db.sql("""
		select
			posting_date, account, debit, credit, is_opening, fiscal_year, debit_in_account_currency, credit_in_account_currency, account_currency
		from `tabGL Entry`
		where
			company=%(company)s
			{additional_conditions}
	...
	if filters.get('presentation_currency'):
		gle = convert_to_presentation_currency(gle, currency_map)

	for d in gle:
		opening.setdefault(d.account,[]).append(d)
...
def calculate_values(accounts, gl_entries_by_account, opening_balances, filters, company_currency):
	...
	for d in accounts:
		...
		# add opening
		# frappe.msgprint(gl_entries_by_account.get(d.name, []))
		for o in opening_balances.get(d.name, []):
			d["opening_debit"] += flt(o.debit)
			d["opening_credit"] += flt(o.credit)
