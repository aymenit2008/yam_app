# Copyright (c) 2013, aymenit2008@gmail.com and contributors
...
def get_gl_entries(filters):
	...
	select_fields = """, debit, credit, debit_in_account_currency,
		credit_in_account_currency, debit_in_usd, credit_in_usd, debit_in_eur, credit_in_eur, debit_in_gbp, credit_in_gbp """

	order_by_statement = "order by debit desc, posting_date, account, creation"

	if filters.get("group_by") == _("Group by Voucher"):
		order_by_statement = "order by debit desc, posting_date, voucher_type, voucher_no"
	...
	gl_entries = frappe.db.sql(
		...
		select
			...
			voucher_type, voucher_no, sun_cost_center, cost_center, project, project_activities, customer, currency,
...
def get_conditions(filters):
	...
	if filters.get("root_type"):
		conditions.append("account in (select name from tabAccount where root_type=%(root_type)s )")
...
def group_by_field(group_by):
	if group_by == _('Group by Party'):
		return 'party'
	elif group_by in [_('Group by Account')]:
		return 'account'
	elif group_by in [_('Group by Cost Center')]:
		return 'cost_center'
	else:
		return 'voucher_no'
